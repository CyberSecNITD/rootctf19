...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($err)){
        ...
        $sql = "SELECT user_id, email, password,user_agent FROM Users WHERE email = ? AND user_agent= ?";

   
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("ss", $param_email,$user_agent);
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){                    
                    ...
                    $stmt->bind_result($id, $email, $hashed_password,$registered_user_agent);

                    echo $hashed_password . '<br>' ;
                    if($stmt->fetch()){
                        if(password_verify($password, $hashed_password)){

                            if($registered_user_agent == $user_agent){
                                ...
                                $_SESSION["user_agent"] = $registered_user_agent ;                           
                                ...
                                echo "<script> window.alert('The same registered user agent, try from another browser') </script>  " ;
                            }
                            ...
                            echo "<script> window.alert('Good Luck') ; </script>" ;
                        }
                    }
                } else{
